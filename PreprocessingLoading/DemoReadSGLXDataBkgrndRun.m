...
% much easier!
% This is from the SpikeGLX Github:
% 
% https://billkarsh.github.io/SpikeGLX/#post-processing-tools
%
% function DemoReadSGLXDataBkgrndRun()
...
TimeLengthRecording=meta.fileTimeSecs;
% TimeLengthRecording='420';
for ch=1:385
    dataArray=[];
    nSamp = floor(1.0 * SampRate(meta));
    TTL = [];
    samp0 = 0
    for i=1:str2num(TimeLengthRecording)
        i
        dataArray1 = ReadBin(samp0, nSamp, meta, binName, path);
        samp0 = samp0+nSamp;
        ...
        digArray = ExtractDigital(dataArray1, meta, dw, dLineList);
...
%         end
    ...
    SampleRate=SampRate(meta);
    if contains(binName,'.lf.bin')==1 && dataType == 'A'
        mkdir([path,'\ExtractedMatlabData'])
        SampleRateLFP=SampleRate;
        save([path,'\ExtractedMatlabData\LFPChannelPerRecording_',num2str(ch)],'dataArray','meta','SampleRateLFP')
    end
    if contains(binName,'.ap.bin')==1 && dataType == 'A'
        mkdir([path,'\ExtractedMatlabData'])
         SampleRateAP=SampleRate;
        save([path,'\ExtractedMatlabData\APChannelPerRecording_',num2str(ch)],'dataArray','meta','SampleRateAP')
    end
end
% DemoReadSGLXData
% end
